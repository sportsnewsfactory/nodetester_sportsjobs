...
export const GENERALNEWS = {
     ...
     */
    async getTransItemsByLang(DB: MYSQL_DB, lang: string): Promise<DB.Item.GeneralNews[]> {
        // console.log(`getGeneralNewsItems`);
        const funcName = `NEWS.getGeneralNewsItems`;
        
        try {
            const sql = `
                SELECT rn.id, tr.headline, tr.sub_headline, tr.narration, rn.sport_id, sports.name as sport_name, rn.background, rn.logo, rn.show_standings, rn.league_season_id, tr.lang
            ...
            const items = itemsResult[0] as DB.Item.GeneralNews[];
            return items.sort((a, b) => Number(a.id) - Number(b.id)).slice(0, 5);
        } catch (e) {
