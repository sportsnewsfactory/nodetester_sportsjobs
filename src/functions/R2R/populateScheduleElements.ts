...
export function populateScheduleElements(
    schedule: Motorsport.Schedule.List[],
    ...
    try {
        ...
        for (let i=0; i<schedule.length; i++){
            const numItem = i+1;
            const eventList: Motorsport.Schedule.List = schedule[i];
            
            // we'll currently ignore header
            // and sub_header
            for (let j=0; j<eventList.events.length; j++){
                const event = eventList.events[j];
                const numEntry = j+1;
